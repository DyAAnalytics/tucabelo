...
function renderDynamicProducts(filterCategory = null) {
  ...
  grid.innerHTML = productsToShow.map((p, index) => {
    ...
    const discount = hasSale ? Math.round((1 - Number(p.salePrice) / Number(p.price)) * 100) : 0;
    const priceBlock = hasSale
      ? `<span class="price-sale">${formatPrice(p.salePrice)}</span><span class="price-original">${formatPrice(p.price)}</span><span class="price-badge">-${discount}%</span>`
      : `<span class="price-current">${formatPrice(p.price)}</span>`;
    const offerBadge = hasSale ? `<span class="product-offer-badge">OFERTA</span>` : '';

    return `
      <article class="product-card reveal reveal-delay-${delay} visible" data-price="${hasSale ? p.salePrice : p.price}" data-name="${p.name}">
        ...
        <div class="product-info">
          ...
          <div class="product-actions">
            <button class="btn-add-cart" onclick="addToCart('${p.id}','${p.name.replace(/'/g, "\\'")}',${hasSale ? p.salePrice : p.price},'${p.image}','${p.category}')">🛒 Agregar</button>
  ...
  // Update catalog count elements (catalogo.html)
  const totalEl = document.getElementById('catalog-total-count');
  const resultsEl = document.getElementById('catalog-results-count');
  if (totalEl) totalEl.textContent = globalProducts.length;
  if (resultsEl) resultsEl.textContent = `${productsToShow.length} producto${productsToShow.length !== 1 ? 's' : ''}`;
}
